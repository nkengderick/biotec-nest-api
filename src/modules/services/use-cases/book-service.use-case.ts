...
export class BookingService {
  ...
  const service = await this.serviceRepository.findById(service_id.toString());
  if (!service) {
  ...
  const text = `
    ...
    - Price: ${pricingPlan ? '$' + pricingPlan.price : 'Free'}
  ...
  const html = `
    ...
    <ul>
      ...
      <li><strong>Price:</strong> ${pricingPlan ? '$' + pricingPlan.price : 'Free'}</li>
